...
The getRpcUrl() function only supports the listed supported EVM chains [here](/sdk/getting-started/prerequisites/supported-evm-chains). The returned RPC URLs are public RPC endpoints that can be subject to throttling and rate limiting, so consider using your own RPC provider for more reliable access.
