...
Raw contract interactions for PKP permissions are verbose and easy to misconfigure. This manager abstracts encoding, idempotent updates, and normalised reads into a single, ergonomic API. It reduces integration risk, enforces least‑privilege by design, and offers helper checks and snapshots so applications can present and mutate permissions confidently.
...
const removeScopeTx = await pkpPermissionsManager.removePermittedAuthMethodScope({
  authMethodType: 1,
  authMethodId: '0x1234567890abcdef1234567890abcdef12345678',
  scopeId: 1,
});
